...
class ControllerExtensionModulePsPopupDialogBox extends Controller
{
    ...
    public function index($setting)
    {
        ...
        if ($this->request->server['HTTPS']) {
            $data['bg_image'] = isset($setting['bg_image'][$language_id]['image']) ? $this->config->get('config_ssl') . 'image/' . html_entity_decode($setting['bg_image'][$language_id]['image'], ENT_QUOTES, 'UTF-8') : '';
        } else {
